...
public interface WPObject extends Serializable, Cloneable {
    ...
    /**
     * Get the material to place at the specified relative coordinates. Should only
     * be invoked for coordinates for which
     * {@link #getMask(int, int, int)} returns {@code true}. These coordinates are
     * zero-based and must never be
     * negative.
     * 
     ...
     * @param y The relative Y coordinate.
     * @param z The relative Z coordinate.
     * @return {@code true} if a block should be placed at the specified relative
     *         coordinates.
     */
     ...
     *
     * <p>
     * <strong>Note</strong> that this method calls {@link #getDimensions()}
    ...
    default Point3i guestimateOffset() {
        ...
        return (offsetZ > Integer.MIN_VALUE)
                ? new Point3i(-(lowestX + highestX) / 2, -(lowestY + highestY) / 2, -offsetZ)
                : null;
    }
    ...
    /**
     * Will guess whether the {@link #ATTRIBUTE_CONNECT_BLOCKS} attribute should be
     * set and if so set it. Should only be
     * called for custom object formats which have legacy, numerical block IDs, for
     * which you might want to set that
     * property.
     */
    default void guessConnectBlocks() {
        ...
        visitBlocks((object, x, y, z, material) -> {
            if (material.connectingBlock) {
                // First check if one of the connecting properties is set; if so then this
                // object must contain modern
                // materials and may be assumed to have the right properties set already. This
                // is probably redundant.
                if (material.is(WEST) || material.is(NORTH) || material.is(EAST) || material.is(SOUTH)) {
                    setConnectBlocks.set(false);
                    return false;
                } else {
                    // The material has none of its connecting properties set. Check if there is a
                    // block around it in
                    ...
                    if (((x > 0) && getMask(x - 1, y, z) && wouldConnect(material, getMaterial(x - 1, y, z)))
                            ...
                            || ((x < (dim.x - 1)) && getMask(x + 1, y, z)
                                    && wouldConnect(material, getMaterial(x + 1, y, z)))
                            || ((y < (dim.y - 1)) && getMask(x, y + 1, z)
                                    && wouldConnect(material, getMaterial(x, y + 1, z)))) {
                        // There is a block around to which this block should have been connected, so
                        // conclude for now
                        ...
                        case 2:
                            for (int x = 0; x < dim.x; x++) {
                                ...
                                if (entities.containsKey(pos)) {
                                    ...
                                    System.out.printf("|%d:%4.4s", entitiesEncountered.size(),
    ...
    /**
     * Random rotation <strong>and</strong> mirroring, for historical reasons. See
     * {@link #ATTRIBUTE_RANDOM_ROTATION_ONLY}
     * and {@link #ATTRIBUTE_RANDOM_MIRRORING_ONLY} for separate attributes. If one
     * or both of those attributes are
     * set, this one must be set to {@code false}, since otherwise it defaults to
     * {@code true} and will override the
     * separate attributes.
     */
    ...
    AttributeKey<Boolean> ATTRIBUTE_SPAWN_ON_WATER_NO_COLLIDE = new AttributeKey<>("WPObject.spawnOnWater.noCollide",
            false);
    /**
     ...
     * </tr>
     * <tr>
     * <td>{@link #COLLISION_MODE_ALL}</td>
     * <td>Will collide with (and therefore not render) any above ground block other
     * than air</td>
     * </tr>
     * <tr>
     * <td><strong>{@link #COLLISION_MODE_SOLID}</strong></td>
     * <td>Will collide with (and therefore not render) any above ground
     * <em>solid</em> block (i.e. not air, grass, water, flowers, leaves, etc.).
     * Default value</td>
     * </tr>
     * <tr>
     * <td>{@link #COLLISION_MODE_NONE}</td>
     * <td>Will not collide with <em>any</em> above ground block (and therefore
     * intersect any other object already there!)</td>
     * </tr>
     * </table>
     */
    AttributeKey<Integer> ATTRIBUTE_COLLISION_MODE = new AttributeKey<>("WPObject.collisionMode", COLLISION_MODE_SOLID); // See
                                                                                                                         // COLLISION_MODE_*
                                                                                                                         // constants
    /**
     ...
     * </tr>
     * <tr>
     * <td><strong>{@link #COLLISION_MODE_ALL}</strong></td>
     * <td>Every underground block belonging to the object will be rendered
     * (including air blocks), regardless of what is already there. Default
     * value</td>
     * </tr>
     * <tr>
     * <td>{@link #COLLISION_MODE_SOLID}</td>
     * <td>Every <em>solid</em> (i.e. not air, grass, water, flowers, leaves, etc.)
     * underground block belonging to the object will be rendered regardless of what
     * is already there. Non-solid blocks will be rendered only if the existing
     * block is air</td>
     * </tr>
     * <tr>
     * <td>{@link #COLLISION_MODE_NONE}</td>
     * <td>Underground blocks belonging to the object will only be rendered if the
     * existing block is air</td>
     * </tr>
     * </table>
     */
    AttributeKey<Integer> ATTRIBUTE_UNDERGROUND_MODE = new AttributeKey<>("WPObject.undergroundMode",
            COLLISION_MODE_ALL); // See COLLISION_MODE_* constants
    /**
     * Whether to change leaf blocks so that they do or do not decay. Possible
     * values:
     * 
     * <p>
     * <table>
     ...
     * </tr>
     * <tr>
     * <td><strong>{@link #LEAF_DECAY_NO_CHANGE}</strong></td>
     * <td>Leaf blocks are copied unchanged from the custom object. Default
     * value</td>
     * </tr>
     * <tr>
     * <td>{@link #LEAF_DECAY_ON}</td>
     * <td>All leaf blocks are set to decay regardless of their setting in the
     * custom object</td>
     * </tr>
     * <tr>
     * <td>{@link #LEAF_DECAY_OFF}</td>
     * <td>All leaf blocks are set to <em>not</em> decay regardless of their setting
     * in the custom object</td>
     * </tr>
     * </table>
     */
    AttributeKey<Integer> ATTRIBUTE_LEAF_DECAY_MODE = new AttributeKey<>("WPObject.leafDecay", LEAF_DECAY_NO_CHANGE); // See
                                                                                                                      // LEAF_DECAY_*
                                                                                                                      // constants
    /**
    ...
    /**
     * Whether certain connecting blocks such as fences, glass panes and iron bars
     * should have their west, north, east
     * and south properties automatically set if there is a connecting block in that
     * direction. Does not currently
